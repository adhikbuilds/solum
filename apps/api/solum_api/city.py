...
database instead of three generated files. Two of those files simply stop existing:
  ...
  `plots.pmtiles`      -> tiles cut on request by `solum_city.db.tiles`, cached in Postgres
...
import solum_city.db
from solum_city.db import tiles as tilesvc
...
# Resolved through the package that owns it, not by counting directories up from this file.
SCHEMA = Path(solum_city.db.__file__).resolve().parent / 'schema.sql'
