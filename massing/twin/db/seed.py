...
A city in a file: fill an empty database without a network fetch.
    ...
    python -m twin.db.seed                    # load the committed seed into Postgres
    python -m twin.db.seed --warm             # ...and build the z8-12 overview tiles
    python -m twin.db.seed --extract          # regenerate the seed from a full snapshot

The problem this solves is the first five minutes of a fresh clone. `twin/raw/` is gitignored --
206 MB of dated ArcGIS pages that nobody should carry in git -- so `python -m twin.db.load` on a
...
from twin.db import load as loader                                     # noqa: E402
from twin.pipeline.city import _mass_one                               # noqa: E402
...
def extract(source_aoi: str = 'dubai-all', path: Path = DEFAULT_SEED) -> int:
    """
    Re-cut the seed from a full snapshot on disk. Needs `twin/raw/`; developers regenerating the
    seed have it, everyone else just loads the committed result.
    """
    from twin.sources import parcels

    snap = parcels.latest(source_aoi)
    if snap is None:
        print(f'! no snapshot on disk for {source_aoi}; run `python -m twin.pipeline.city fetch`')
        return 1
