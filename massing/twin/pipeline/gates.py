...
from twin.model import config as cityconfig
from twin.sources import parcels as acquire                      # noqa: E402
from twin.pipeline.build import OUT_DIR                            # noqa: E402
...
def run(city: str = 'dubai') -> tuple[list[Gate], Path]:
    ...
    features = snap.features() if snap else acquire.from_fixture(
        Path(__file__).resolve().parents[3] / 'fixtures' / 'parcel-3156315.json')
