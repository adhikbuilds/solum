...
is the chain -- geometry is acquired in EPSG:3997 and every attribute keeps the rung it came from.
    ...
    python -m twin.pipeline.city fetch     # ~101 pages from DDA into raw/dubai-all/<date>/
    python -m twin.pipeline.city geojson   # snapshot -> out/dubai-all/plots.geojsonl (4326)
    python -m twin.pipeline.city massing   # the derived scheme per plot, in parallel (~20 min)
    python -m twin.pipeline.city tiles     # geojsonl -> out/dubai-all/plots.pmtiles
    python -m twin.pipeline.city all
...
def geojson() -> int:
    ...
    if snap is None:
        print(f'! no snapshot for {aoi}; run `python -m twin.pipeline.city fetch` first')
        return 1
...
def tiles() -> int:
    ...
    if not src.exists():
        print(f'! {src} missing; run `python -m twin.pipeline.city geojson` first')
        return 1
    ...
    if mass.exists():
        ...
        print('  (no massing.geojsonl; run `python -m twin.pipeline.city massing` for schemes)')
